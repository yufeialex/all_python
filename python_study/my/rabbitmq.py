...
logging.basicConfig(filename='/opt/logs/zabbix/rabbitmq_zabbix.log', level=logging.WARNING,
...
def main():
    ...
    if options.check == 'list_queues':
        print
        json.dumps({'data': api.list_queues()}, indent=4, separators=(',', ':'))
    elif options.check == 'list_nodes':
        print
        json.dumps({'data': api.list_nodes()}, indent=4, separators=(',', ':'))
        ...
        if not options.metric:
            ...
            if options.node:
                print
                api.check_overview(options.metric)
            else:
                print
                api.check_overview(options.metric)
        ...
        if not options.metric:
            ...
            if options.node:
                print
                api.check_server(options.metric, options.node)
            else:
                print
                api.check_server(options.metric, api.host_name)
